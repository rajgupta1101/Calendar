...
Author: Raj Gupta
*/
/* The following code prints the calendar of the year input by the user */
...
int main()
{
	...
	while (i <= 12)
	{
		...
		//Print tab until the day corresponding to 01/../.... is reached
		f(k, 1, n)
				cout
			<< "\t";

		//If Current month is February, set number of days as determined in line 72:75
		if (i == 2)
			d = febday;

		//Run 'for loop' from 1 to d+1 where d is the number of days in that month
		f(k, 1, d + 1)
		{
			//Print dates while adjusting space for two digit numbers
			if (k < 10)
				cout << k << "   \t";
			...
			n++;
			//Reset day of the week to Monday (i.e. 1) if it becomes greater than Sunday (i.e. 7)
			if (n > 7)
			{
				n = 1;
				cout << "\n";
